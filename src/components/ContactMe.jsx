...
const ContactMe = ({ handleCloseContactForm, isContactModal, t }) => {
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        'service_d3wy84h', 'template_lak2vsn',
        e.target,
        'PQyyV5GLEN7wlHrmH'
      )
      .then(
    ...
    >
      <section className="bg-slate-950 dark:bg-zinc-900 w-[min(100%,_700px)] flex justify-center flex-col items-center gap-2 p-4 relative text-white">
        <span className="absolute top-2 right-2" onClick={handleCloseContactForm}>
          <i className="bx bx-x hover:rotate-180 cursor-pointer text-3xl transition-all hover:bg-zinc-900 bg-yellow-300 dark:bg-blue-600 rounded-full w-9 aspect-square text-center"></i>
        </span>
        ...
        >
          ...
          <div>
            <input
              className="bg-yellow-700 px-4 hover:bg-zinc-900 dark:bg-blue-600 hover:scale-105 cursor-pointer transition-all"
              type="submit"
              value={t("eng.btnSend")}
            />
          </div>
